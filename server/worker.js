  ...
  try {
    const response = await axios.get(url, {
      ...
      onDownloadProgress: (progressEvent) => {
        // This is the progress callback of axios which will be called for every progress update
        let percentCompleted = Math.round(
          (progressEvent.loaded * 100) / progressEvent.total
        ); // Calculate the percentage of the download completed to update the progress to the parent thread
        progressEvent.progress &&
          parentPort.postMessage({ progress: percentCompleted }); // Send the progress to the parent thread if the loaded property exists
      },
    });
    ...
    if (response.status !== 200) {
      ...
      const destinationAndFilenameWithExtension =
        destination + finalFileExtension; // Append the extracted file extension to the destination path received from the parent thread
