...
let bars = {}; // This will hold the progress bar instances for each worker thread
let errorBars = {}; // This will hold the progress bar instances for each worker thread when the download fails
const workerList = []; // This will hold the worker threads
...
const processDownloadQueue = () => {
  ...
  while (downloadQueue.length > 0 && activeDownloads < maxConcurrentDownloads) {
    ...
    workerList.push(worker); // Push the worker thread to the workerList array
    ...
    worker.on("message", (message) => {
      ...
      if (message.progress !== undefined) {
        ...
        errorBars[worker.threadId] = bars[worker.threadId]; // Store the progress bar in the errorBars object
  ...
  if (downloadQueue.length === 0 && activeDownloads === 0) {
    //Cleanup
    workerList.forEach((worker) => worker.unref()); // Unref all the worker threads
    workerList.forEach((worker) => worker.terminate()); // Terminate all the worker threads
    multibar.stop(); // Stop the progress bar
    Object.values(bars).forEach((bar) => bar.update(100)); // Complete the progress bar for the failed downloads to remove them from the console
    Object.keys(errorBars).forEach((key) => {
      console.error(`Download failed for worker ${key}`); // Log a message to the console for each failed download
    });
    errorBars = {}; // Reset the errorBars object
    bars = {}; // Reset the bars object
